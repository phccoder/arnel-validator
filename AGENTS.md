...
- **Column roles are user-selectable** via `RoleCheckMatrix` (row per column, checkbox per role) for **Match / Sum / Date / Type**. Match, Date and Type are single-select (enforced with the just-clicked column winning); Sum is multi-select. The role checkboxes are pre-checked from `detect_column_roles` but can be changed manually. `on_continue` warns (with a continue/cancel prompt) if no Date column is picked so the date range is never silently ignored.
